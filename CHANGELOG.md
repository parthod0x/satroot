...
- Adds `python -m satroot_profile_federation_smoke`, `satroot-profile-federation-smoke`, and `scripts/run_profile_federation_smoke.py` for a first `v0.4-publication-federation` operator wrapper that reuses the released profile matrix, freezes the resulting per-profile workspaces into collections, and proves a mixed-profile publication stack plus network above them.
- Adds the packaged federation smoke module to installed-import verification, packaging assertions, release guidance, and local ignore rules so the new `v0.4` proof surface is easy to run from both source and editable installs.
