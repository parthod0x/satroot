...
- Implements `jcs-n` from `draft-mih-sokolov-scitt-payload-binding-01` and measures it against SATROOT's digest, in `satroot_jcs` and `docs/CANONICALISATION.md`. The two disagree on the ordinary SATROOT state snapshot, not on exotic input: `jcs-n` recursively removes members whose value is null or an empty collection, which strips `frozen_accounts`, `profile`, `profile_mode` and an unset `max_supply` from a snapshot SATROOT retains them in. The schemes encode different answers to whether an absent member equals an empty one - both defensible, but a profile digesting under one and verifying under the other fails in a way that looks like corruption. Also records that the "n" in `jcs-n` is absent-field normalisation, not Unicode normalisation.
