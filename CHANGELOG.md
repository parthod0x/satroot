...
- Adds `python -m satroot_profile_federation_smoke`, `satroot-profile-federation-smoke`, and `scripts/run_profile_federation_smoke.py` for a first `v0.4-publication-federation` operator wrapper that reuses the released profile matrix, freezes the resulting per-profile demo-catalog, publication-stack, publication-network, publication-catalog-workspace, and publication-registry-workspace outputs into collections, and proves a mixed-profile publication stack plus network above them.
