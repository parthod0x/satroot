...
from satroot1 import (
    bootstrap_stable_demo_bundle_index_from_presets,
...
def _load_json_object(path: str | Path) -> dict[str, Any]:
    ...
    if not isinstance(payload, dict):
        raise ValueError("stable demo bundle-index smoke preset input must be a JSON object")
    return payload
...
def _stage_stable_preset_variants(base_preset_json: str | Path, output_dir: Path) -> list[Path]:
    base_preset = _load_json_object(base_preset_json)
    output_dir.mkdir(parents=True, exist_ok=True)

    variants = [
        (
            "stable_bundle_alpha.json",
            "SBIDX01",
            "Stable Bundle Alpha",
            {
                "reference_unit": "EUR",
                "intended_use": "alpha-bundle-ledger",
            },
        ),
        (
            "stable_bundle_beta.json",
            "SBIDX02",
            "Stable Bundle Beta",
            {
                "reference_unit": "GBP",
                "intended_use": "beta-bundle-ledger",
            },
        ),
    ]
...
def run_stable_demo_bundle_index_smoke(
    output_dir: str | Path,
    *,
    ...
    staged_preset_paths = _stage_stable_preset_variants(
        base_preset_json,
    ...
    generated = bootstrap_stable_demo_bundle_index_from_presets(
        ...
        bundle_index_metadata_overrides={
            "channel": "stable",
            "label": "Stable Demo Bundle Index Smoke",
    ...
    report_path = output_path / "stable_demo_bundle_index_smoke_report.json"
...
def parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Run the stable demo bundle-index convenience flow from staged presets "
            "and emit a compact verification report."
        )
    )
    parser.add_argument(
        "--output-dir",
        default=".tmp_stable_demo_bundle_index_smoke_run",
        help="Directory where generated stable bundles, the bundle collection, bundle index, and report will be written.",
    )
    parser.add_argument(
        "--base-preset-json",
        default=str(DEFAULT_BASE_PRESET_JSON),
        help="Checked-in stable demo catalog preset used as the source template for staged smoke variants.",
    )
    parser.add_argument(
        "--bundle-scheme",
        default="hmac-sha256",
        help="Signing scheme for generated stable demo bundles.",
    )
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    report = run_stable_demo_bundle_index_smoke(
        args.output_dir,
