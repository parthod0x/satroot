...
from satroot1 import (
    bootstrap_stable_demo_release_catalog_index_publication_from_presets,
...
def _load_json_object(path: str | Path) -> dict[str, Any]:
    ...
    if not isinstance(payload, dict):
        raise ValueError("stable demo release catalog index smoke preset input must be a JSON object")
    return payload
...
def run_stable_demo_release_catalog_index_smoke(
    output_dir: str | Path,
    *,
    ...
    generated = bootstrap_stable_demo_release_catalog_index_publication_from_presets(
        staged_preset_paths,
        ...
        catalog_metadata={
            "channel": "stable",
            "label": "Stable Demo Release Catalog Smoke",
            "published_at": "2026-08-18T00:00:00Z",
        },
        index_metadata={
            "channel": "stable",
            "label": "Stable Demo Release Catalog Index Smoke",
            "published_at": "2026-08-18T00:00:00Z",
        },
    )
    ...
    report_path = output_path / "stable_demo_release_catalog_index_smoke_report.json"
...
def parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Run the stable demo release-catalog index publication convenience flow "
            "from staged presets and emit a compact verification report."
        )
    )
    parser.add_argument(
        "--output-dir",
        default=".tmp_stable_demo_release_catalog_index_smoke_run",
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    report = run_stable_demo_release_catalog_index_smoke(
        args.output_dir,
