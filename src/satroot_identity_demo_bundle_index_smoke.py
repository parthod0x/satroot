...
def _load_json_object(path: str | Path) -> dict[str, Any]:
    ...
    if not isinstance(payload, dict):
        raise ValueError("identity demo bundle-index smoke preset input must be a JSON object")
    return payload
...
def _stage_identity_preset_variants(base_preset_json: str | Path, output_dir: Path) -> list[Path]:
    base_preset = _load_json_object(base_preset_json)
    output_dir.mkdir(parents=True, exist_ok=True)

    variants = [
        (
            "identity_bundle_alpha.json",
            "IDBIDX01",
            "Identity Bundle Alpha",
            {
                "identity_type": "service-identity",
                "subject_id": "ops-alpha",
                "controller_entity": "operations-alpha",
                "authority_scope": "admin-alpha",
                "intended_use": "identity-bundle-alpha",
            },
            {
                "holder_account": "identity_holder_alpha",
                "next_holder": "identity_next_alpha",
                "retire": False,
            },
        ),
        (
            "identity_bundle_beta.json",
            "IDBIDX02",
            "Identity Bundle Beta",
            {
                "identity_type": "service-identity",
                "subject_id": "ops-beta",
                "controller_entity": "operations-beta",
                "authority_scope": "admin-beta",
                "intended_use": "identity-bundle-beta",
            },
            {
                "holder_account": "identity_holder_beta",
                "next_holder": "identity_next_beta",
                "retire": False,
            },
        ),
    ]
...
def run_identity_demo_bundle_index_smoke(
    output_dir: str | Path,
    *,
    ...
    staged_preset_paths = _stage_identity_preset_variants(
        base_preset_json,
    ...
    generated = bootstrap_singleton_demo_bundle_index_from_presets(
        ...
        bundle_index_metadata_overrides={
            "channel": "identity",
            "label": "Identity Demo Bundle Index Smoke",
    ...
    report_path = output_path / "identity_demo_bundle_index_smoke_report.json"
...
def parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Run the identity singleton demo bundle-index convenience flow from "
            "staged presets and emit a compact verification report."
        )
    )
    parser.add_argument(
        "--output-dir",
        default=".tmp_identity_demo_bundle_index_smoke_run",
        help="Directory where generated identity bundles, the bundle collection, bundle index, and report will be written.",
    )
    parser.add_argument(
        "--base-preset-json",
    ...
    parser.add_argument(
        "--bundle-scheme",
        default="hmac-sha256",
        help="Signing scheme for generated identity demo bundles.",
    )
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    report = run_identity_demo_bundle_index_smoke(
        args.output_dir,
