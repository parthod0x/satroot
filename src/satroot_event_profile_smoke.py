...
DEFAULT_EVENTS_JSON = EXAMPLES_ROOT / "events_event1.json"
DEFAULT_PRESET_JSON = EXAMPLES_ROOT / "catalog_presets" / "event_stream_catalog.json"
    ...
    *,
    ...
    workspace = bootstrap_singleton_demo_publication_registry_workspace_from_presets(
        ...
        release_metadata_overrides={
            "channel": "event",
            "label": "Event Profile Smoke Release",
        },
        release_catalog_metadata={
            "channel": "event",
            "label": "Event Profile Smoke Release Catalog",
        },
        release_catalog_index_metadata={
            "channel": "event",
            "label": "Event Profile Smoke Release Catalog Index",
        },
        descriptor_index_metadata={
            "channel": "event",
            "label": "Event Profile Smoke Descriptor Index",
        },
        publication_metadata_catalog_metadata={
            "channel": "event",
            "label": "Event Profile Smoke Metadata Catalog",
        },
        publication_registry_metadata={
            "channel": "event",
            "label": "Event Profile Smoke Registry",
        },
    )
    ...
    if len(generated_catalog_dirs) != 1:
        raise ValueError("event profile smoke expected exactly one generated singleton demo catalog workspace")
    ...
    report["ok"] = all(
        [
            ...
            report["ledger_replay"]["profile_mode"] == "single-stream",
...
def parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Replay the checked-in event-stream ledger example and generate a full "
            "SATROOT-EVENT-1 publication registry workspace smoke report."
        )
    )
    parser.add_argument(
        "--output-dir",
        default=".tmp_event_profile_smoke_run",
        help="Directory where the generated event publication registry workspace and report will be written.",
    )
    parser.add_argument(
        "--events-json",
        default=str(DEFAULT_EVENTS_JSON),
        help="Event-stream example events JSON to replay before publication generation.",
    )
    parser.add_argument(
        "--preset-json",
        default=str(DEFAULT_PRESET_JSON),
        help="Event demo catalog preset used to generate the singleton publication registry workspace.",
    )
    parser.add_argument(
        "--bundle-scheme",
        default="hmac-sha256",
