...
def _load_bundle_index_artifact(
    bundle_index_dir: str | Path,
) -> tuple[Path, Dict[str, Any]]:
    candidate_path = Path(bundle_index_dir).resolve()
    if candidate_path.is_dir():
        bundle_index_path = candidate_path / "bundle_index.json"
        if not bundle_index_path.is_file():
            raise SatRootError("bundle_index.json is required for bundle index operations")
    elif candidate_path.name == "bundle_index.json":
        bundle_index_path = candidate_path
    elif candidate_path.name == "release_manifest.json":
        manifest = _load_json_object_file(str(candidate_path), label="release-manifest")
        ...
        bundle_index_path = (candidate_path.parent / bundle_index_ref).resolve()
        if not bundle_index_path.is_file():
            raise SatRootError(f"bundle index file not found: {bundle_index_ref}")
    else:
        raise SatRootError("bundle index operations require a bundle_index.json file, release directory, or release_manifest.json")
    ...
    if not isinstance(index, dict):
        raise SatRootError("bundle index must contain an object")
    validate_bundle_index_consistency(index)
    return bundle_index_path, index


def summarize_bundle_index_artifact(bundle_index_dir: str | Path) -> Dict[str, Any]:
    bundle_index_path, index = _load_bundle_index_artifact(bundle_index_dir)
    bundles = index.get("bundles")
    assert isinstance(bundles, list)
    return {
        ...
        "bundle_root_ids": sorted({str(entry.get("root_id")) for entry in bundles}),
        "bundles": copy.deepcopy(bundles),
    }


def lint_bundle_index_artifact(bundle_index_dir: str | Path) -> Dict[str, Any]:
    bundle_index_path, index = _load_bundle_index_artifact(bundle_index_dir)
    bundles = index.get("bundles")
    assert isinstance(bundles, list)
    ...
    for entry in bundles:
        ...
        resolved_bundle_dir = (bundle_index_path.parent / bundle_path_ref).resolve()
        resolved_manifest_path = (bundle_index_path.parent / manifest_path_ref).resolve()
...
def summarize_signed_release_publication(release_dir: str | Path) -> Dict[str, Any]:
    ...
    summary = summarize_bundle_index_artifact(bundle_index_path)
    return {
        "signature_scheme": manifest.get("signature_scheme"),
        "signature_key_id": manifest.get("signature_key_id"),
        ...
        "bundle_root_ids": summary.get("bundle_root_ids"),
...
def lint_signed_release_publication(release_dir: str | Path) -> Dict[str, Any]:
    ...
    bundles = index.get("bundles")
    assert isinstance(bundles, list)
    bundle_index_report = lint_bundle_index_artifact(bundle_index_path)
    ...
    return {
        ...
        "bundle_count": index.get("bundle_count"),
        "duplicate_bundle_ids": bundle_index_report["duplicate_bundle_ids"],
        "duplicate_bundle_paths": bundle_index_report["duplicate_bundle_paths"],
        "duplicate_manifest_paths": bundle_index_report["duplicate_manifest_paths"],
        "bundle_manifest_path_mismatches": bundle_index_report["bundle_manifest_path_mismatches"],
        "missing_bundle_directories": bundle_index_report["missing_bundle_directories"],
        "missing_bundle_manifests": bundle_index_report["missing_bundle_manifests"],
        "manifest_hash_mismatches": bundle_index_report["manifest_hash_mismatches"],
        "bundle_manifest_metadata_mismatches": bundle_index_report["bundle_manifest_metadata_mismatches"],
    }
...
def discover_signed_publication_descriptor_index_publication_dirs(
    search_roots: Sequence[str | Path],
    *,
    recursive: bool = True,
) -> list[str]:
    if not search_roots:
        raise SatRootError("at least one publication descriptor index discovery root is required")

    discovered: Dict[str, str] = {}
    for search_root in search_roots:
        root_path = Path(search_root).resolve()
        if not root_path.exists():
            raise SatRootError(f"publication descriptor index discovery root not found: {search_root}")
        if not root_path.is_dir():
            raise SatRootError(f"publication descriptor index discovery root must be a directory: {search_root}")

        manifest_paths = (
            root_path.rglob("publication_descriptor_index_manifest.json")
            if recursive
            else root_path.glob("publication_descriptor_index_manifest.json")
        )
        for manifest_path in manifest_paths:
            publication_dir = str(manifest_path.parent.resolve())
            discovered.setdefault(publication_dir, publication_dir)

    if not discovered:
        raise SatRootError("no publication descriptor index directories found under the provided discovery roots")
    return sorted(discovered.values())


def discover_signed_publication_metadata_catalog_publication_dirs(
    search_roots: Sequence[str | Path],
    *,
    recursive: bool = True,
) -> list[str]:
    if not search_roots:
        raise SatRootError("at least one publication metadata catalog discovery root is required")

    discovered: Dict[str, str] = {}
    for search_root in search_roots:
        root_path = Path(search_root).resolve()
        if not root_path.exists():
            raise SatRootError(f"publication metadata catalog discovery root not found: {search_root}")
        if not root_path.is_dir():
            raise SatRootError(f"publication metadata catalog discovery root must be a directory: {search_root}")

        manifest_paths = (
            root_path.rglob("publication_metadata_catalog_manifest.json")
            if recursive
            else root_path.glob("publication_metadata_catalog_manifest.json")
        )
        for manifest_path in manifest_paths:
            publication_dir = str(manifest_path.parent.resolve())
            discovered.setdefault(publication_dir, publication_dir)
    ...
    if not discovered:
        raise SatRootError("no publication metadata catalog directories found under the provided discovery roots")
    return sorted(discovered.values())
...
def discover_publication_catalog_workspace_dirs(
    search_roots: Sequence[str | Path],
    *,
    recursive: bool = True,
) -> list[str]:
    return _discover_workspace_dirs(
        ...
        summary_validator=validate_publication_catalog_workspace_summary_consistency,
    )
...
def summarize_publication_metadata_catalog_publication(publication_metadata_catalog_dir: str | Path) -> Dict[str, Any]:
    _, catalog_path, manifest, catalog = _load_publication_metadata_catalog_publication(publication_metadata_catalog_dir)
    bundles = catalog.get("bundles")
    assert isinstance(bundles, list)
    return {
        "signature_scheme": manifest.get("signature_scheme"),
        "signature_key_id": manifest.get("signature_key_id"),
        "publication_metadata_catalog_path": manifest.get("publication_metadata_catalog_path"),
        "publication_metadata_catalog_hash": manifest.get("publication_metadata_catalog_hash"),
        "publication_metadata_catalog_resolved_path": str(catalog_path),
        "bundle_count": catalog.get("bundle_count"),
        "index": copy.deepcopy(catalog.get("index")),
        "artifact_paths": sorted(str(entry.get("artifact_path")) for entry in bundles),
        "artifact_kinds": sorted(
            {
                str(entry.get("artifact_kind"))
                for entry in bundles
...
def lint_publication_metadata_catalog_publication(publication_metadata_catalog_dir: str | Path) -> Dict[str, Any]:
    manifest_path, catalog_path, manifest, catalog = _load_publication_metadata_catalog_publication(publication_metadata_catalog_dir)
    bundles = catalog.get("bundles")
    assert isinstance(bundles, list)

    actual_catalog_hash = "sha256:" + sha256_hex_bytes(catalog_path.read_bytes())
    publication_metadata_catalog_hash_matches = manifest.get("publication_metadata_catalog_hash") == actual_catalog_hash
    bundle_count_matches = manifest.get("bundle_count") == catalog.get("bundle_count")
    index_metadata_matches = manifest.get("index") == catalog.get("index")

    bundle_id_counts: Dict[str, int] = {}
    bundle_path_counts: Dict[str, int] = {}
    manifest_path_counts: Dict[str, int] = {}
    report_path_counts: Dict[str, int] = {}
    descriptor_path_counts: Dict[str, int] = {}
    for entry in bundles:
        bundle_id = entry.get("publication_metadata_bundle_id")
        bundle_path_ref = entry.get("publication_metadata_bundle_path")
        manifest_path_ref = entry.get("publication_metadata_manifest_path")
        report_path_ref = entry.get("publication_report_path")
        descriptor_path_ref = entry.get("publication_descriptor_path")
        if isinstance(bundle_id, str):
            bundle_id_counts[bundle_id] = bundle_id_counts.get(bundle_id, 0) + 1
        if isinstance(bundle_path_ref, str):
            bundle_path_counts[bundle_path_ref] = bundle_path_counts.get(bundle_path_ref, 0) + 1
        if isinstance(manifest_path_ref, str):
            manifest_path_counts[manifest_path_ref] = manifest_path_counts.get(manifest_path_ref, 0) + 1
        if isinstance(report_path_ref, str):
            report_path_counts[report_path_ref] = report_path_counts.get(report_path_ref, 0) + 1
        if isinstance(descriptor_path_ref, str):
            descriptor_path_counts[descriptor_path_ref] = descriptor_path_counts.get(descriptor_path_ref, 0) + 1

    duplicate_bundle_ids = sorted(value for value, count in bundle_id_counts.items() if count > 1)
    duplicate_bundle_paths = sorted(value for value, count in bundle_path_counts.items() if count > 1)
    duplicate_manifest_paths = sorted(value for value, count in manifest_path_counts.items() if count > 1)
    duplicate_report_paths = sorted(value for value, count in report_path_counts.items() if count > 1)
    duplicate_descriptor_paths = sorted(value for value, count in descriptor_path_counts.items() if count > 1)

    bundle_manifest_path_mismatches: list[str] = []
    missing_bundle_directories: list[str] = []
    missing_bundle_manifests: list[str] = []
    missing_publication_reports: list[str] = []
    missing_publication_descriptors: list[str] = []
    publication_metadata_manifest_hash_mismatches: list[str] = []
    publication_report_hash_mismatches: list[str] = []
    publication_descriptor_hash_mismatches: list[str] = []
    publication_metadata_bundle_mismatches: list[Dict[str, Any]] = []
    ...
    for entry in bundles:
        ...
        if not isinstance(manifest_path_ref, str) or not manifest_path_ref.strip():
            continue
        if not isinstance(report_path_ref, str) or not report_path_ref.strip():
            continue
        if not isinstance(descriptor_path_ref, str) or not descriptor_path_ref.strip():
            continue

        expected_manifest_path = (
            "publication_metadata_manifest.json"
            if bundle_path_ref in {"", "."}
            else f"{bundle_path_ref}/publication_metadata_manifest.json"
        )
        if manifest_path_ref != expected_manifest_path:
            bundle_manifest_path_mismatches.append(bundle_path_ref)
        ...
        resolved_report_path = (manifest_path.parent / report_path_ref).resolve()
        resolved_descriptor_path = (manifest_path.parent / descriptor_path_ref).resolve()
        if not resolved_bundle_dir.is_dir():
            missing_bundle_directories.append(bundle_path_ref)
        if not resolved_manifest_path.is_file():
            missing_bundle_manifests.append(manifest_path_ref)
        ...
        if not resolved_report_path.is_file():
            missing_publication_reports.append(report_path_ref)
            continue
        if not resolved_descriptor_path.is_file():
            missing_publication_descriptors.append(descriptor_path_ref)
            continue

        actual_manifest_hash = "sha256:" + sha256_hex_bytes(resolved_manifest_path.read_bytes())
        actual_report_hash = "sha256:" + sha256_hex_bytes(resolved_report_path.read_bytes())
        actual_descriptor_hash = "sha256:" + sha256_hex_bytes(resolved_descriptor_path.read_bytes())
        if entry.get("publication_metadata_manifest_hash") != actual_manifest_hash:
            publication_metadata_manifest_hash_mismatches.append(manifest_path_ref)
        if entry.get("publication_report_hash") != actual_report_hash:
            publication_report_hash_mismatches.append(report_path_ref)
        if entry.get("publication_descriptor_hash") != actual_descriptor_hash:
            publication_descriptor_hash_mismatches.append(descriptor_path_ref)

        _nested_manifest_path, nested_report_path, nested_descriptor_path, nested_manifest, nested_descriptor = (
            _load_publication_metadata_bundle_publication(resolved_bundle_dir)
        )
        mismatched_fields = [
            field_name
            for field_name in [
            ...
            if entry.get(field_name) != nested_manifest.get(field_name)
        ]
        if resolved_manifest_path != _nested_manifest_path and "publication_metadata_manifest_path" not in mismatched_fields:
            mismatched_fields.append("publication_metadata_manifest_path")
        if resolved_report_path != nested_report_path and "publication_report_path" not in mismatched_fields:
            mismatched_fields.append("publication_report_path")
        if resolved_descriptor_path != nested_descriptor_path and "publication_descriptor_path" not in mismatched_fields:
            mismatched_fields.append("publication_descriptor_path")
        if entry.get("artifact_kind") != nested_descriptor.get("artifact_kind") and "artifact_kind" not in mismatched_fields:
            mismatched_fields.append("artifact_kind")
        if entry.get("artifact_path") != nested_descriptor.get("artifact_path") and "artifact_path" not in mismatched_fields:
            mismatched_fields.append("artifact_path")

        if mismatched_fields:
            publication_metadata_bundle_mismatches.append(
                {
                    "publication_metadata_bundle_path": bundle_path_ref,
                    "fields": sorted(set(mismatched_fields)),
                }
            )
    ...
    return {
        "ok": not any(
            [
                not publication_metadata_catalog_hash_matches,
                not bundle_count_matches,
                not index_metadata_matches,
                duplicate_bundle_ids,
                duplicate_bundle_paths,
                duplicate_manifest_paths,
                duplicate_report_paths,
                duplicate_descriptor_paths,
                bundle_manifest_path_mismatches,
                missing_bundle_directories,
                missing_bundle_manifests,
                missing_publication_reports,
                missing_publication_descriptors,
                publication_metadata_manifest_hash_mismatches,
                publication_report_hash_mismatches,
                publication_descriptor_hash_mismatches,
                publication_metadata_bundle_mismatches,
            ]
        ),
        "signature_scheme": manifest.get("signature_scheme"),
        "signature_key_id": manifest.get("signature_key_id"),
        "publication_metadata_catalog_path": manifest.get("publication_metadata_catalog_path"),
        "publication_metadata_catalog_hash_matches": publication_metadata_catalog_hash_matches,
        "bundle_count_matches": bundle_count_matches,
        "index_metadata_matches": index_metadata_matches,
        "declared_bundle_count": len(bundles),
        "bundle_count": catalog.get("bundle_count"),
        ...
        "duplicate_descriptor_paths": duplicate_descriptor_paths,
        "bundle_manifest_path_mismatches": sorted(bundle_manifest_path_mismatches),
        "missing_bundle_directories": sorted(missing_bundle_directories),
        "missing_bundle_manifests": sorted(missing_bundle_manifests),
        "missing_publication_reports": sorted(missing_publication_reports),
        "missing_publication_descriptors": sorted(missing_publication_descriptors),
        "publication_metadata_manifest_hash_mismatches": sorted(publication_metadata_manifest_hash_mismatches),
        "publication_report_hash_mismatches": sorted(publication_report_hash_mismatches),
        "publication_descriptor_hash_mismatches": sorted(publication_descriptor_hash_mismatches),
        "publication_metadata_bundle_mismatches": publication_metadata_bundle_mismatches,
    }
...
def _require_machine_release_publication(release_dir: str | Path, *, label: str) -> None:
    ...
    unexpected_profiles: set[Any] = set()
    for entry in bundles:
        if not isinstance(entry, Mapping):
            raise SatRootError(f"{label} has an invalid bundle entry")
        manifest_path_ref = entry.get("manifest_path")
        if not isinstance(manifest_path_ref, str) or not manifest_path_ref.strip():
            raise SatRootError(f"{label} is missing nested bundle manifest metadata")
        manifest_path = (bundle_index_path.parent / manifest_path_ref).resolve()
        if not manifest_path.is_file():
            raise SatRootError(f"{label} bundle manifest not found: {manifest_path_ref}")
        bundle_manifest = _load_validated_bundle_manifest(manifest_path.parent)
        final_snapshot = bundle_manifest.get("final_state_snapshot")
        profile = final_snapshot.get("profile") if isinstance(final_snapshot, Mapping) else None
        if profile != MACHINE_DEMO_CATALOG_PROFILE:
            unexpected_profiles.add(profile)

    if unexpected_profiles or not bundles:
        raise SatRootError(
            f"{label} must contain only {MACHINE_DEMO_CATALOG_PROFILE} bundles; "
            f"found {sorted(unexpected_profiles) or 'none'}"
        )


def resolve_machine_release_directory_inputs(
    release_dirs: Sequence[str | Path],
    *,
    discover_under: Optional[Sequence[str | Path]] = None,
    recursive: bool = True,
    ...
    resolved = resolve_release_directory_inputs(
        release_dirs,
        discover_under=discover_under,
        recursive=recursive,
    )
    for release_dir in resolved:
        _require_machine_release_publication(release_dir, label=label)
    return resolved
...
def _require_machine_satroot_artifact_path(
    ...
    *,
    ...
    if artifact_kind == "release-catalog-index":
        ...
        return
    if artifact_kind == "publication-descriptor-index":
        _require_machine_publication_descriptor_index_publication(resolved_path, label=label)
        return
    if artifact_kind == "publication-metadata-catalog":
        _require_machine_publication_metadata_catalog_publication(resolved_path, label=label)
        return
...
def validate_publication_catalog_workspace_summary_consistency(summary: Mapping[str, Any]) -> None:
    ...
    descriptor_index_dir = summary.get("publication_descriptor_index_dir")
    metadata_bundles_dir = summary.get("publication_metadata_bundles_dir")
    metadata_catalog_dir = summary.get("publication_metadata_catalog_dir")
    descriptor_index_manifest_path = summary.get("publication_descriptor_index_manifest_path")
    metadata_catalog_manifest_path = summary.get("publication_metadata_catalog_manifest_path")
    if not isinstance(artifact_paths, list):
        raise SatRootError("publication catalog workspace summary artifact_paths must be an array")
    if not isinstance(metadata_bundles, list):
        raise SatRootError("publication catalog workspace summary publication_metadata_bundles must be an array")
    if not isinstance(artifact_count, int) or artifact_count != len(artifact_paths):
        raise SatRootError("publication catalog workspace summary artifact_count mismatch")
    if not isinstance(metadata_bundle_count, int) or metadata_bundle_count != len(metadata_bundles):
        raise SatRootError("publication catalog workspace summary publication_metadata_bundle_count mismatch")
    if not isinstance(descriptor_index_dir, str) or not descriptor_index_dir.strip():
        raise SatRootError("publication catalog workspace summary publication_descriptor_index_dir must be a non-empty string")
    if not isinstance(metadata_bundles_dir, str) or not metadata_bundles_dir.strip():
        raise SatRootError("publication catalog workspace summary publication_metadata_bundles_dir must be a non-empty string")
    if not isinstance(metadata_catalog_dir, str) or not metadata_catalog_dir.strip():
        raise SatRootError("publication catalog workspace summary publication_metadata_catalog_dir must be a non-empty string")
    if not isinstance(descriptor_index_manifest_path, str) or not descriptor_index_manifest_path.strip():
        raise SatRootError("publication catalog workspace summary publication_descriptor_index_manifest_path must be a non-empty string")
    if not isinstance(metadata_catalog_manifest_path, str) or not metadata_catalog_manifest_path.strip():
        raise SatRootError("publication catalog workspace summary publication_metadata_catalog_manifest_path must be a non-empty string")
    if not isinstance(summary.get("publication_descriptor_index"), Mapping):
        raise SatRootError("publication catalog workspace summary publication_descriptor_index must be an object")
    if not isinstance(summary.get("publication_metadata_catalog"), Mapping):
        raise SatRootError("publication catalog workspace summary publication_metadata_catalog must be an object")
...
def validate_publication_registry_workspace_summary_consistency(summary: Mapping[str, Any]) -> None:
    ...
    if not isinstance(release_catalog_index_dir, str) or not release_catalog_index_dir.strip():
        raise SatRootError("publication registry workspace summary release_catalog_index_dir must be a non-empty string")
    if not isinstance(publication_registry_dir, str) or not publication_registry_dir.strip():
        raise SatRootError("publication registry workspace summary publication_registry_dir must be a non-empty string")
    if not isinstance(publication_registry_manifest_path, str) or not publication_registry_manifest_path.strip():
        raise SatRootError("publication registry workspace summary publication_registry_manifest_path must be a non-empty string")
    if not isinstance(summary.get("publication_registry"), Mapping):
        raise SatRootError("publication registry workspace summary publication_registry must be an object")


def _resolve_publication_catalog_workspace_component_dirs(workspace_path: Path) -> Dict[str, Path]:
    return {
...
def lint_demo_catalog_workspace(demo_catalog_dir: str | Path) -> Dict[str, Any]:
    ...
    for entry in bundles:
        ...
        genesis_metadata = snapshot.get("genesis_metadata")
        ...
        if isinstance(profile_fields, dict):
            for field_name, field_value in profile_fields.items():
                if field_name in snapshot:
                    actual_value = snapshot.get(field_name)
                else:
                    actual_value = genesis_metadata.get(field_name)
        ...
        if isinstance(structure_overrides, dict):
            for field_name, field_value in structure_overrides.items():
                if field_name in snapshot:
                    actual_value = snapshot.get(field_name)
                elif field_name in genesis_metadata:
                    actual_value = genesis_metadata.get(field_name)
                else:
                    continue
                if actual_value != field_value:
...
def summarize_publication_catalog_workspace(publication_catalog_workspace_dir: str | Path) -> Dict[str, Any]:
...
def lint_publication_catalog_workspace(publication_catalog_workspace_dir: str | Path) -> Dict[str, Any]:
    workspace_path, summary = _load_workspace_summary(publication_catalog_workspace_dir, label="publication catalog workspace")
    validate_publication_catalog_workspace_summary_consistency(summary)
    metadata_bundles = summary.get("publication_metadata_bundles")
    artifact_paths = summary.get("artifact_paths")
...
def bootstrap_machine_credit_demo_catalog_workspace(
    *,
    ...
    return write_demo_catalog_workspace(
        ...
        profile_field_overrides={"SATROOT-MACHINE-1": resolved_profile_fields},
        profile_structure_overrides={"SATROOT-MACHINE-1": resolved_structure_overrides},
        key_prefix=key_prefix,
        key_suffix=key_suffix,
        include_state_hash=include_state_hash,
        include_annotation=include_annotation,
...
def bootstrap_machine_credit_publication_catalog_workspace(
    *,
    ...
    summary["source_machine_catalog_workspace_dir"] = str(machine_catalog_workspace_dir.resolve())
    summary["source_machine_catalog_summary_path"] = machine_catalog_workspace["summary_path"]
    summary["source_machine_catalog_release_dir"] = machine_catalog_workspace["summary"]["release_dir"]
    summary["source_machine_catalog_release_manifest_path"] = machine_catalog_workspace["summary"]["release_manifest_path"]
    _write_json_file(root_output_dir / "summary.json", summary)

    publication_catalog_workspace["summary"] = summary
    publication_catalog_workspace["machine_catalog_workspace_dir"] = str(machine_catalog_workspace_dir.resolve())
    publication_catalog_workspace["machine_catalog_workspace"] = machine_catalog_workspace
...
def publish_machine_release_catalog(
    ...
    *,
    ...
    resolved_release_dirs = [Path(value).resolve() for value in release_dirs]
    if not resolved_release_dirs:
        raise SatRootError("machine release catalog publishing requires at least one SATROOT-MACHINE-1 release")

    for release_dir in resolved_release_dirs:
        _require_machine_release_publication(
            release_dir,
            label="machine release catalog publishing source release",
        )

    return publish_signed_release_catalog(
        resolved_release_dirs,
        output_dir=output_dir,
        signature_scheme=signature_scheme,
        key_id=key_id,
        signer=signer,
...
def bootstrap_machine_release_catalog_publication(
    release_dirs: Sequence[str | Path],
    *,
    output_dir: str | Path,
    signature_scheme: str,
    ...
    return bootstrap_release_catalog_publication(
        resolved_release_dirs,
        output_dir=output_dir,
        signature_scheme=signature_scheme,
        key_id=key_id,
...
def write_publication_registry_workspace(
    *,
    ...
    summary["release_catalog_index_dir"] = str(copied_release_catalog_index_dir.resolve())
    summary["publication_registry_dir"] = str(registry_dir.resolve())
    summary["publication_registry_manifest_path"] = publication_registry_publication["publication_registry_manifest_path"]
    summary["publication_registry"] = copy.deepcopy(publication_registry_publication["publication_registry"])
    summary_path = root_output_dir / "summary.json"
    _write_json_file(summary_path, summary)
    return {
        "summary": summary,
        ...
        "publication_descriptor_index_dir": str(descriptor_index_dir),
        "publication_metadata_bundles_dir": str(metadata_bundles_dir),
        "publication_metadata_catalog_dir": str(metadata_catalog_dir),
        "publication_registry_dir": str(registry_dir.resolve()),
        "publication_descriptor_index_publication": catalog_workspace["publication_descriptor_index_publication"],
        "publication_metadata_bundle_collection": catalog_workspace["publication_metadata_bundle_collection"],
        "publication_metadata_catalog_publication": catalog_workspace["publication_metadata_catalog_publication"],
...
def inventory_workspace_artifacts(
    ...
    *,
    ...
    publication_descriptor_index_dirs = _discover_optional_paths(
        discover_signed_publication_descriptor_index_publication_dirs,
        resolved_search_roots,
        recursive=recursive,
    )
    publication_metadata_catalog_dirs = _discover_optional_paths(
        discover_signed_publication_metadata_catalog_publication_dirs,
        resolved_search_roots,
        recursive=recursive,
    )
    ...
    publication_catalog_workspace_dirs = _discover_optional_paths(
        discover_publication_catalog_workspace_dirs,
        resolved_search_roots,
        recursive=recursive,
    )
    ...
    publication_descriptor_index_entries: list[Dict[str, Any]] = []
    for publication_dir in publication_descriptor_index_dirs:
        descriptor_index_summary = summarize_publication_descriptor_index_publication(publication_dir)
        publication_descriptor_index_entries.append(
            {
                "publication_descriptor_index_dir": str(Path(publication_dir).resolve()),
                "signature_scheme": descriptor_index_summary.get("signature_scheme"),
                "signature_key_id": descriptor_index_summary.get("signature_key_id"),
                "artifact_count": descriptor_index_summary.get("artifact_count"),
                "artifact_kinds": copy.deepcopy(descriptor_index_summary.get("artifact_kinds")),
                "index": copy.deepcopy(descriptor_index_summary.get("index")),
            }
        )

    publication_metadata_catalog_entries: list[Dict[str, Any]] = []
    for publication_dir in publication_metadata_catalog_dirs:
        metadata_catalog_summary = summarize_publication_metadata_catalog_publication(publication_dir)
        publication_metadata_catalog_entries.append(
            {
                "publication_metadata_catalog_dir": str(Path(publication_dir).resolve()),
                "signature_scheme": metadata_catalog_summary.get("signature_scheme"),
                "signature_key_id": metadata_catalog_summary.get("signature_key_id"),
                "bundle_count": metadata_catalog_summary.get("bundle_count"),
                "artifact_kinds": copy.deepcopy(metadata_catalog_summary.get("artifact_kinds")),
                "index": copy.deepcopy(metadata_catalog_summary.get("index")),
            }
        )
    ...
    publication_catalog_workspace_entries: list[Dict[str, Any]] = []
    for workspace_dir in publication_catalog_workspace_dirs:
        workspace_summary = summarize_publication_catalog_workspace(workspace_dir)
        publication_catalog_workspace_entries.append(
            {
                "workspace_dir": str(Path(workspace_dir).resolve()),
                ...
                "publication_descriptor_index": copy.deepcopy(workspace_summary.get("publication_descriptor_index_summary", {}).get("index")),
                "publication_metadata_catalog": copy.deepcopy(workspace_summary.get("publication_metadata_catalog", {}).get("index")),
            }
        )
    ...
    return {
        ...
        "publication_descriptor_index_count": len(publication_descriptor_index_entries),
        "publication_metadata_catalog_count": len(publication_metadata_catalog_entries),
        ...
        "publication_descriptor_indexes": publication_descriptor_index_entries,
        "publication_metadata_catalogs": publication_metadata_catalog_entries,
        ...
        "publication_catalog_workspaces": publication_catalog_workspace_entries,
...
def _detect_satroot_artifact_kind(path: str | Path) -> tuple[str, Path]:
    ...
    if resolved_path.is_file():
        ...
        if name == "publication_descriptor_index_manifest.json":
        ...
        if name == "summary.json":
            ...
            if (parent / "publication_descriptor_index").is_dir() and (parent / "publication_metadata_catalog").is_dir():
                return "publication-catalog-workspace", parent
    ...
    if (resolved_path / "summary.json").is_file():
        ...
        if (resolved_path / "publication_descriptor_index").is_dir() and (resolved_path / "publication_metadata_catalog").is_dir():
            return "publication-catalog-workspace", resolved_path
...
def build_satroot_artifact_descriptor(path: str | Path) -> Dict[str, Any]:
    ...
    if kind == "publication-descriptor-index":
        summary = summarize_publication_descriptor_index_publication(artifact_path)
        descriptor.update(
            {
                "signature_scheme": summary.get("signature_scheme"),
                "signature_key_id": summary.get("signature_key_id"),
                "artifact_count": summary.get("artifact_count"),
                "index": copy.deepcopy(summary.get("index")),
                "artifact_kinds": copy.deepcopy(summary.get("artifact_kinds")),
                "artifact_paths": copy.deepcopy(summary.get("artifact_paths")),
            }
        )
        return descriptor

    if kind == "publication-metadata-catalog":
        summary = summarize_publication_metadata_catalog_publication(artifact_path)
        descriptor.update(
            {
                "signature_scheme": summary.get("signature_scheme"),
                "signature_key_id": summary.get("signature_key_id"),
    ...
    if kind == "publication-catalog-workspace":
        summary = summarize_publication_catalog_workspace(artifact_path)
        descriptor.update(
            {
                "signature_scheme": summary.get("signature_scheme"),
                "artifact_count": summary.get("artifact_count"),
                ...
                "publication_metadata_catalog": copy.deepcopy((summary.get("publication_metadata_catalog") or {}).get("index")),
            }
        )
        return descriptor
...
def validate_publication_descriptor_consistency(descriptor: Mapping[str, Any]) -> None:
    ...
    if artifact_kind not in {
        ...
        "publication-descriptor-index",
        "publication-metadata-catalog",
        "demo-catalog",
        "publication-stack",
        "publication-network",
...
def discover_satroot_artifact_paths(
    ...
    *,
    ...
    for artifact_path in _discover_optional_paths(discover_signed_publication_descriptor_index_publication_dirs, resolved_search_roots, recursive=recursive):
        add_artifact("publication-descriptor-index", artifact_path)
    for artifact_path in _discover_optional_paths(discover_signed_publication_metadata_catalog_publication_dirs, resolved_search_roots, recursive=recursive):
        add_artifact("publication-metadata-catalog", artifact_path)
    ...
    for artifact_path in _discover_optional_paths(discover_publication_catalog_workspace_dirs, resolved_search_roots, recursive=recursive):
        add_artifact("publication-catalog-workspace", artifact_path)
...
def render_satroot_artifact_report(path: str | Path) -> str:
    ...
    if kind == "publication-descriptor-index":
        ...
        lines.extend(
            [
                "# SATROOT Publication Descriptor Index Report",
                "",
                f"- Path: `{artifact_path}`",
                f"- Signature scheme: `{summary.get('signature_scheme')}`",
                f"- Signature key ID: `{summary.get('signature_key_id')}`",
        ...
        if isinstance(artifact_kinds, list):
            ...
            lines.extend(f"- `{value}`" for value in artifact_kinds if isinstance(value, str))
        lines.append("")
        return "\n".join(lines)
    ...
    if kind == "publication-metadata-catalog":
        ...
        lines.extend(
            [
                "# SATROOT Publication Metadata Catalog Report",
                "",
                f"- Path: `{artifact_path}`",
                f"- Signature scheme: `{summary.get('signature_scheme')}`",
                f"- Signature key ID: `{summary.get('signature_key_id')}`",
    ...
    if kind == "publication-catalog-workspace":
        summary = summarize_publication_catalog_workspace(artifact_path)
        publication_descriptor_index_summary = summary.get("publication_descriptor_index_summary")
        publication_metadata_catalog = summary.get("publication_metadata_catalog")
        lines.extend(
            [
                "# SATROOT Publication Catalog Workspace Report",
                "",
                f"- Path: `{artifact_path}`",
                f"- Signature scheme: `{summary.get('signature_scheme')}`",
                f"- Artifact count: `{summary.get('artifact_count')}`",
        ...
        if isinstance(publication_metadata_catalog, Mapping):
            index_metadata = publication_metadata_catalog.get("index")
            if isinstance(index_metadata, Mapping):
                _append_metadata_lines(lines, index_metadata, [("channel", "Channel"), ("label", "Label"), ("published_at", "Published at")])
        ...
        for label, component_summary, count_field in (
            ("Publication Descriptor Index", publication_descriptor_index_summary, "artifact_count"),
            ("Publication Metadata Catalog", publication_metadata_catalog, "bundle_count"),
        ):
...
def build_cli_parser() -> Any:
    ...
    bootstrap_machine_demo_catalog_parser.add_argument("--consumption-model", default="burn-on-use", help="Compact machine consumption model metadata")
    bootstrap_machine_demo_catalog_parser.add_argument("--root-id", help="Optional explicit root_id; defaults to a generated placeholder root")
    bootstrap_machine_demo_catalog_parser.add_argument("--issuer", default="issuer", help="Issuer account name for genesis and tenant allocation events")
    bootstrap_machine_demo_catalog_parser.add_argument("--tenant-account", default="tenant_a", help="Tenant account receiving the primary machine-credit allocation")
    bootstrap_machine_demo_catalog_parser.add_argument("--worker-account", default="worker_node", help="Machine worker account receiving consumable execution credits")
    bootstrap_machine_demo_catalog_parser.add_argument("--max-supply", help="Optional explicit max_supply override; defaults to the initial issued balance")
    bootstrap_machine_demo_catalog_parser.add_argument("--initial-balance", default="100000000", help="Initial issued balance allocated to the issuer")
    bootstrap_machine_demo_catalog_parser.add_argument("--tenant-amount", default="5000000", help="Amount transferred from the issuer to the tenant account")
    bootstrap_machine_demo_catalog_parser.add_argument("--worker-amount", default="1200000", help="Amount transferred from the tenant account to the worker account")
    bootstrap_machine_demo_catalog_parser.add_argument("--worker-burn-amount", default="200000", help="Optional worker-side burn amount; use 0 to skip the burn event")
    bootstrap_machine_demo_catalog_parser.add_argument("--intended-use", default="machine-api-credit", help="Compact machine intended_use metadata")
    bootstrap_machine_demo_catalog_parser.add_argument("--profile-field", action="append", dest="profile_fields", help="Additional machine profile field override in key=value form; may be repeated")
    bootstrap_machine_demo_catalog_parser.add_argument("--rules-hash", help="Optional rules_hash metadata")
    bootstrap_machine_demo_catalog_parser.add_argument("--nonce", help="Optional nonce metadata")
    bootstrap_machine_demo_catalog_parser.add_argument("--key-prefix", default="", help="Optional prefix for generated key IDs")
    bootstrap_machine_demo_catalog_parser.add_argument("--key-suffix", default="-key", help="Optional suffix for generated key IDs")
    bootstrap_machine_demo_catalog_parser.add_argument("--no-state-hash", action="store_true", help="Do not attach state_hash during bundle signing")
    bootstrap_machine_demo_catalog_parser.add_argument("--no-annotated-output", action="store_true", help="Do not emit annotated_signed_events.json")
    bootstrap_machine_demo_catalog_parser.add_argument("--verifier-only", action="store_true", help="For ed25519 bundles, omit private_keys.json and emit verifier-only material")
    bootstrap_machine_demo_catalog_parser.add_argument("--channel", help="Optional release channel metadata for the bundle index")
    bootstrap_machine_demo_catalog_parser.add_argument("--label", help="Optional human-readable release label for the bundle index")
    ...
    bootstrap_machine_publication_catalog_workspace_parser.add_argument("--output-dir", required=True, help="Directory where machine_catalog_workspace/, publication_descriptor_index/, publication_metadata_bundles/, publication_metadata_catalog/, and summary.json will be written")
    bootstrap_machine_publication_catalog_workspace_parser.add_argument("--service-scope", default="api-compute", help="Compact machine service scope metadata")
    bootstrap_machine_publication_catalog_workspace_parser.add_argument("--billing-unit", default="request", help="Compact machine billing unit metadata")
    bootstrap_machine_publication_catalog_workspace_parser.add_argument("--consumption-model", default="burn-on-use", help="Compact machine consumption model metadata")
    bootstrap_machine_publication_catalog_workspace_parser.add_argument("--root-id", help="Optional explicit root_id; defaults to a generated placeholder root")
    bootstrap_machine_publication_catalog_workspace_parser.add_argument("--issuer", default="issuer", help="Issuer account name for genesis and tenant allocation events")
    bootstrap_machine_publication_catalog_workspace_parser.add_argument("--tenant-account", default="tenant_a", help="Tenant account receiving the primary machine-credit allocation")
    bootstrap_machine_publication_catalog_workspace_parser.add_argument("--worker-account", default="worker_node", help="Machine worker account receiving consumable execution credits")
    bootstrap_machine_publication_catalog_workspace_parser.add_argument("--max-supply", help="Optional explicit max_supply override; defaults to the initial issued balance")
    bootstrap_machine_publication_catalog_workspace_parser.add_argument("--initial-balance", default="100000000", help="Initial issued balance allocated to the issuer")
    bootstrap_machine_publication_catalog_workspace_parser.add_argument("--tenant-amount", default="5000000", help="Amount transferred from the issuer to the tenant account")
    bootstrap_machine_publication_catalog_workspace_parser.add_argument("--worker-amount", default="1200000", help="Amount transferred from the tenant account to the worker account")
    bootstrap_machine_publication_catalog_workspace_parser.add_argument("--worker-burn-amount", default="200000", help="Optional worker-side burn amount; use 0 to skip the burn event")
    bootstrap_machine_publication_catalog_workspace_parser.add_argument("--intended-use", default="machine-api-credit", help="Compact machine intended_use metadata")
    bootstrap_machine_publication_catalog_workspace_parser.add_argument("--profile-field", action="append", dest="profile_fields", help="Additional machine profile field override in key=value form; may be repeated")
    bootstrap_machine_publication_catalog_workspace_parser.add_argument("--rules-hash", help="Optional rules_hash metadata")
    bootstrap_machine_publication_catalog_workspace_parser.add_argument("--nonce", help="Optional nonce metadata")
    bootstrap_machine_publication_catalog_workspace_parser.add_argument("--key-prefix", default="", help="Optional prefix for generated key IDs")
    bootstrap_machine_publication_catalog_workspace_parser.add_argument("--key-suffix", default="-key", help="Optional suffix for generated key IDs")
    bootstrap_machine_publication_catalog_workspace_parser.add_argument("--no-state-hash", action="store_true", help="Do not attach state_hash during bundle signing")
    bootstrap_machine_publication_catalog_workspace_parser.add_argument("--no-annotated-output", action="store_true", help="Do not emit annotated_signed_events.json")
    bootstrap_machine_publication_catalog_workspace_parser.add_argument("--verifier-only", action="store_true", help="For ed25519 bundles, omit private_keys.json and emit verifier-only material")
    bootstrap_machine_publication_catalog_workspace_parser.add_argument("--channel", help="Optional release channel metadata for the nested machine catalog bundle index")
    bootstrap_machine_publication_catalog_workspace_parser.add_argument("--label", help="Optional human-readable release label for the nested machine catalog bundle index")
    bootstrap_machine_publication_catalog_workspace_parser.add_argument("--published-at", help="Optional ISO-8601 style published-at metadata for the nested machine catalog bundle index")
    bootstrap_machine_publication_catalog_workspace_parser.add_argument("--descriptor-index-channel", help="Optional descriptor-index channel metadata")
    bootstrap_machine_publication_catalog_workspace_parser.add_argument("--descriptor-index-label", help="Optional human-readable descriptor-index label")
    bootstrap_machine_publication_catalog_workspace_parser.add_argument("--descriptor-index-published-at", help="Optional descriptor-index published_at metadata")
    bootstrap_machine_publication_catalog_workspace_parser.add_argument("--publication-metadata-catalog-channel", help="Optional publication-metadata-catalog channel metadata")
    ...
    bootstrap_publication_catalog_workspace_parser.add_argument("--descriptor-index-published-at", help="Optional descriptor-index published_at metadata")
    bootstrap_publication_catalog_workspace_parser.add_argument("--publication-metadata-catalog-channel", help="Optional publication-metadata-catalog channel metadata")
    bootstrap_publication_catalog_workspace_parser.add_argument("--publication-metadata-catalog-label", help="Optional human-readable publication-metadata-catalog label")
    bootstrap_publication_catalog_workspace_parser.add_argument("--publication-metadata-catalog-published-at", help="Optional publication-metadata-catalog published_at metadata")
    bootstrap_publication_catalog_workspace_parser.add_argument("--scheme", choices=["hmac-sha256", "ed25519"], required=True, help="Signing scheme for generated descriptor and metadata publications")
    bootstrap_publication_catalog_workspace_parser.add_argument("--publication-descriptor-index-key-id", required=True, help="Signature key identifier to generate and use for the publication descriptor index manifest")
    bootstrap_publication_catalog_workspace_parser.add_argument("--publication-metadata-key-id", required=True, help="Signature key identifier to generate and use for each publication metadata manifest")
    bootstrap_publication_catalog_workspace_parser.add_argument("--publication-metadata-catalog-key-id", required=True, help="Signature key identifier to generate and use for the publication metadata catalog manifest")
    bootstrap_publication_catalog_workspace_parser.add_argument("--output-dir", required=True, help="Directory where publication_descriptor_index/, publication_metadata_bundles/, publication_metadata_catalog/, and summary.json will be written")
    ...
    bundle_index_summary_parser = subparsers.add_parser("bundle-index-summary", help="Read bundle_index.json and print a multi-bundle summary without signature verification")
    bundle_index_summary_parser.add_argument("bundle_index_dir", help="Path to a SATROOT bundle_index.json file, release directory, or release_manifest.json file")

    bundle_index_lint_parser = subparsers.add_parser("bundle-index-lint", help="Check bundle_index.json plus referenced bundle manifests without signature verification")
    bundle_index_lint_parser.add_argument("bundle_index_dir", help="Path to a SATROOT bundle_index.json file, release directory, or release_manifest.json file")
    ...
    publication_metadata_catalog_lint_parser = subparsers.add_parser("publication-metadata-catalog-lint", help="Check publication_metadata_catalog_manifest.json, publication_metadata_catalog.json, and referenced publication metadata bundles without signature verification")
    publication_metadata_catalog_lint_parser.add_argument("publication_metadata_catalog_dir", help="Path to a SATROOT publication metadata catalog directory or publication_metadata_catalog_manifest.json/publication_metadata_catalog.json file")
    ...
    publish_machine_release_catalog_parser.add_argument("--channel", help="Optional catalog channel metadata")
    publish_machine_release_catalog_parser.add_argument("--label", help="Optional human-readable catalog label")
    publish_machine_release_catalog_parser.add_argument("--published-at", help="Optional ISO-8601 style published-at metadata for the release catalog")
    publish_machine_release_catalog_parser.add_argument("--scheme", choices=["hmac-sha256", "ed25519"], required=True)
    publish_machine_release_catalog_parser.add_argument("--key-id", required=True, help="Signature key identifier for the release catalog manifest")
    publish_machine_release_catalog_parser.add_argument("--secret", help="Shared secret for hmac-sha256 signing")
    publish_machine_release_catalog_parser.add_argument("--secrets-json", help="Path to JSON mapping key_id -> shared secret for hmac-sha256 release-catalog-manifest signing")
    publish_machine_release_catalog_parser.add_argument("--private-key-hex", help="Hex-encoded Ed25519 private key")
    publish_machine_release_catalog_parser.add_argument("--private-keys-json", help="Path to JSON mapping key_id -> private key hex for ed25519 release-catalog-manifest signing")
    ...
    bootstrap_machine_release_catalog_publication_parser.add_argument("--channel", help="Optional catalog channel metadata")
    bootstrap_machine_release_catalog_publication_parser.add_argument("--label", help="Optional human-readable catalog label")
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    ...
    if args.command == "bootstrap-publication-catalog-workspace":
        ...
        print(f"wrote SATROOT publication catalog workspace to {Path(args.output_dir).resolve()}")
        return 0
    ...
    if args.command == "bundle-index-summary":
        summary = summarize_bundle_index_artifact(args.bundle_index_dir)
        print(canonical_json(summary))
        return 0

    if args.command == "bundle-index-lint":
        report = lint_bundle_index_artifact(args.bundle_index_dir)
        print(canonical_json(report))
        return 0 if report["ok"] else 1
    ...
    if args.command == "publication-metadata-catalog-summary":
        summary = summarize_publication_metadata_catalog_publication(args.publication_metadata_catalog_dir)
        print(canonical_json(summary))
        return 0

    if args.command == "publication-metadata-catalog-lint":
        report = lint_publication_metadata_catalog_publication(args.publication_metadata_catalog_dir)
        print(canonical_json(report))
        return 0 if report["ok"] else 1
    ...
    if args.command == "publication-catalog-workspace-summary":
        summary = summarize_publication_catalog_workspace(args.publication_catalog_workspace_dir)
        print(canonical_json(summary))
        return 0

    if args.command == "publication-catalog-workspace-lint":
        report = lint_publication_catalog_workspace(args.publication_catalog_workspace_dir)
        print(canonical_json(report))
        return 0 if report["ok"] else 1
    ...
    if args.command == "publish-machine-release-catalog":
        ...
        published = publish_machine_release_catalog(
            release_dirs,
            output_dir=args.output_dir,
            signature_scheme=args.scheme,
            key_id=args.key_id,
        ...
        print(f"wrote SATROOT-MACHINE-1 release catalog publication to {Path(published['release_catalog_manifest_path']).parent}")
        return 0
