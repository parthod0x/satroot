...
DEFAULT_EVENTS_JSON = REPO_ROOT / "examples" / "events_license1.json"
DEFAULT_PRESET_JSON = REPO_ROOT / "examples" / "catalog_presets" / "license_usage_catalog.json"
...
def _load_events(events_json: str | Path) -> list[dict[str, Any]]:
    ...
    if not isinstance(payload, list):
        raise ValueError("license profile smoke events input must be a JSON array")
    for index, item in enumerate(payload, start=1):
        if not isinstance(item, dict):
            raise ValueError(f"license profile smoke event {index} must be a JSON object")
    return payload
    ...
    *,
    ...
    workspace = bootstrap_singleton_demo_publication_registry_workspace_from_presets(
        ...
        release_metadata_overrides={
            "channel": "license",
            "label": "License Profile Smoke Release",
        },
        release_catalog_metadata={
            "channel": "license",
            "label": "License Profile Smoke Release Catalog",
        },
        release_catalog_index_metadata={
            "channel": "license",
            "label": "License Profile Smoke Release Catalog Index",
        },
        descriptor_index_metadata={
            "channel": "license",
            "label": "License Profile Smoke Descriptor Index",
        },
        publication_metadata_catalog_metadata={
            "channel": "license",
            "label": "License Profile Smoke Metadata Catalog",
        },
        publication_registry_metadata={
            "channel": "license",
            "label": "License Profile Smoke Registry",
        },
    )
    ...
    if len(generated_catalog_dirs) != 1:
        raise ValueError("license profile smoke expected exactly one generated singleton demo catalog workspace")
    ...
    report: dict[str, Any] = {
        ...
        "ledger_replay": {
            ...
            "license_type": replayed.genesis_metadata.get("license_type"),
            "asset_id": replayed.genesis_metadata.get("asset_id"),
            "usage_scope": replayed.genesis_metadata.get("usage_scope"),
    ...
    report["ok"] = all(
        [
            ...
            report["ledger_replay"]["profile_mode"] == "single-license",
    ...
    report_path = output_path / "license_profile_smoke_report.json"
...
def parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Replay the checked-in license ledger example and generate a full "
            "SATROOT-LICENSE-1 publication registry workspace smoke report."
        )
    )
    parser.add_argument(
        "--output-dir",
        default=".tmp_license_profile_smoke_run",
        help="Directory where the generated license publication registry workspace and report will be written.",
    )
    parser.add_argument(
        "--events-json",
        default=str(DEFAULT_EVENTS_JSON),
        help="License example events JSON to replay before publication generation.",
    )
    parser.add_argument(
        "--preset-json",
        default=str(DEFAULT_PRESET_JSON),
        help="License demo catalog preset used to generate the singleton publication registry workspace.",
    )
    parser.add_argument(
        "--bundle-scheme",
        default="hmac-sha256",
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    report = run_license_profile_smoke(
        args.output_dir,
