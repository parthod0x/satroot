...
from satroot1 import (
    bootstrap_stable_demo_release_catalog_publication_from_presets,
...
def _load_json_object(path: str | Path) -> dict[str, Any]:
    ...
    if not isinstance(payload, dict):
        raise ValueError("stable demo release catalog smoke preset input must be a JSON object")
    return payload
...
def _stage_stable_preset_variants(base_preset_json: str | Path, output_dir: Path) -> list[Path]:
    ...
    variants = [
        (
            "stable_reference_alpha.json",
            "SCATREL01",
            "Stable Catalog Alpha",
            {
                "reference_unit": "EUR",
...
def run_stable_demo_release_catalog_smoke(
    output_dir: str | Path,
    *,
    ...
    report_path = output_path / "stable_demo_release_catalog_smoke_report.json"
...
def parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Run the stable demo release-catalog publication convenience flow from "
            "staged presets and emit a compact verification report."
        )
    )
    parser.add_argument(
        "--output-dir",
        default=".tmp_stable_demo_release_catalog_smoke_run",
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    report = run_stable_demo_release_catalog_smoke(
        args.output_dir,
