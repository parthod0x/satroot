...
from satroot_demo_bundle_index_matrix_smoke import run_demo_bundle_index_matrix_smoke
from satroot_demo_release_catalog_index_matrix_smoke import (
    run_demo_release_catalog_index_matrix_smoke,
)
from satroot_demo_release_catalog_matrix_smoke import run_demo_release_catalog_matrix_smoke
...
def run_publication_ladder_smoke(
    output_dir: str | Path,
    *,
    ...
    bundle_index_report = run_demo_bundle_index_matrix_smoke(
        output_path / "bundle_index",
        bundle_scheme=bundle_scheme,
    )
    release_catalog_report = run_demo_release_catalog_matrix_smoke(
        output_path / "release_catalog",
        bundle_scheme=bundle_scheme,
    )
    release_catalog_index_report = run_demo_release_catalog_index_matrix_smoke(
    ...
    report_path = output_path / "publication_ladder_smoke_report.json"
...
def parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Run the stable and machine publication ladder across the bundle-index, "
            "release-catalog, and release-catalog-index matrix smoke workflows."
        )
    )
    parser.add_argument(
        "--output-dir",
        default=".tmp_publication_ladder_smoke_run",
        help="Directory where publication-ladder layer runs and the consolidated report will be written.",
    )
    parser.add_argument(
        "--bundle-scheme",
        default="hmac-sha256",
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    report = run_publication_ladder_smoke(
        args.output_dir,
