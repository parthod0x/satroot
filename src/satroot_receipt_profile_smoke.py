...
def _load_events(events_json: str | Path) -> list[dict[str, Any]]:
    ...
    if not isinstance(payload, list):
        raise ValueError("receipt profile smoke events input must be a JSON array")
    for index, item in enumerate(payload, start=1):
        if not isinstance(item, dict):
            raise ValueError(f"receipt profile smoke event {index} must be a JSON object")
    return payload
    ...
    *,
    ...
    workspace = bootstrap_singleton_demo_publication_registry_workspace_from_presets(
        ...
        release_metadata_overrides={
            "channel": "receipt",
            "label": "Receipt Profile Smoke Release",
        },
        release_catalog_metadata={
            "channel": "receipt",
            "label": "Receipt Profile Smoke Release Catalog",
        },
        release_catalog_index_metadata={
            "channel": "receipt",
            "label": "Receipt Profile Smoke Release Catalog Index",
        },
        descriptor_index_metadata={
            "channel": "receipt",
            "label": "Receipt Profile Smoke Descriptor Index",
        },
        publication_metadata_catalog_metadata={
            "channel": "receipt",
            "label": "Receipt Profile Smoke Metadata Catalog",
        },
        publication_registry_metadata={
            "channel": "receipt",
            "label": "Receipt Profile Smoke Registry",
        },
    )
    ...
    if len(generated_catalog_dirs) != 1:
        raise ValueError("receipt profile smoke expected exactly one generated singleton demo catalog workspace")
    ...
    report: dict[str, Any] = {
        ...
        "ledger_replay": {
            ...
            "document_type": replayed.genesis_metadata.get("document_type"),
            "reference_id": replayed.genesis_metadata.get("reference_id"),
            "settlement_unit": replayed.genesis_metadata.get("settlement_unit"),
    ...
    report["ok"] = all(
        [
            ...
            report["ledger_replay"]["profile_mode"] == "single-receipt",
    ...
    report_path = output_path / "receipt_profile_smoke_report.json"
...
def parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Replay the checked-in receipt ledger example and generate a full "
            "SATROOT-RECEIPT-1 publication registry workspace smoke report."
        )
    )
    parser.add_argument(
        "--output-dir",
        default=".tmp_receipt_profile_smoke_run",
        help="Directory where the generated receipt publication registry workspace and report will be written.",
    )
    parser.add_argument(
        "--events-json",
        default=str(DEFAULT_EVENTS_JSON),
        help="Receipt example events JSON to replay before publication generation.",
    )
    parser.add_argument(
        "--preset-json",
        default=str(DEFAULT_PRESET_JSON),
        help="Receipt demo catalog preset used to generate the singleton publication registry workspace.",
    )
    parser.add_argument(
        "--bundle-scheme",
        default="hmac-sha256",
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    report = run_receipt_profile_smoke(
        args.output_dir,
