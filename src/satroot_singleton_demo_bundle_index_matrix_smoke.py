...
from satroot_identity_demo_bundle_index_smoke import run_identity_demo_bundle_index_smoke
from satroot_license_demo_bundle_index_smoke import run_license_demo_bundle_index_smoke
from satroot_receipt_demo_bundle_index_smoke import run_receipt_demo_bundle_index_smoke
...
def run_singleton_demo_bundle_index_matrix_smoke(
    output_dir: str | Path,
    *,
    ...
    lane_reports = {
        "receipt": run_receipt_demo_bundle_index_smoke(
            output_path / "receipt",
            bundle_scheme=bundle_scheme,
        ),
        "identity": run_identity_demo_bundle_index_smoke(
            output_path / "identity",
            bundle_scheme=bundle_scheme,
        ),
        "license": run_license_demo_bundle_index_smoke(
            output_path / "license",
            bundle_scheme=bundle_scheme,
        ),
    }
    ...
    report_path = output_path / "singleton_demo_bundle_index_matrix_smoke_report.json"
...
def parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Run the receipt, identity, and license singleton demo bundle-index "
            "smoke workflows and emit one consolidated matrix report."
        )
    )
    parser.add_argument(
        "--output-dir",
        default=".tmp_singleton_demo_bundle_index_matrix_smoke_run",
        help="Directory where per-profile singleton bundle-index smoke runs and the consolidated report will be written.",
    )
    parser.add_argument(
        "--bundle-scheme",
        default="hmac-sha256",
        help="Signature scheme for generated singleton bundle artifacts.",
    )
