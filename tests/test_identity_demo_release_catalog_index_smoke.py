...
def test_run_identity_demo_release_catalog_index_smoke_builds_catalog_and_index(tmp_path):
    report = run_identity_demo_release_catalog_index_smoke(
        tmp_path / "identity_demo_release_catalog_index_smoke"
    )
    ...
    assert report["release_catalog_index_publication"]["index"]["label"] == (
        "Identity Demo Release Catalog Index Smoke"
    )
...
def test_repo_local_identity_demo_release_catalog_index_smoke_wrapper_runs_without_editable_install(
    tmp_path,
):
    ...
    result = subprocess.run(
        [
            sys.executable,
            "scripts/run_identity_demo_release_catalog_index_smoke.py",
            "--output-dir",
...
def test_module_entrypoint_identity_demo_release_catalog_index_smoke_runs_with_repo_pythonpath(
    tmp_path,
):
    repo_root = Path(__file__).resolve().parents[1]
    result = subprocess.run(
