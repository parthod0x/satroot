...
def test_run_singleton_demo_release_catalog_index_matrix_smoke_builds_receipt_identity_license_indexes(
    tmp_path,
):
    report = run_singleton_demo_release_catalog_index_matrix_smoke(
        tmp_path / "singleton_demo_release_catalog_index_matrix_smoke"
    )
    ...
    assert Path(report["profiles"]["receipt"]["release_catalog_index_manifest_path"]).is_file()
    assert Path(report["profiles"]["identity"]["release_catalog_index_manifest_path"]).is_file()
    assert Path(report["profiles"]["license"]["release_catalog_index_manifest_path"]).is_file()
    assert Path(report["report_path"]).is_file()


def test_repo_local_singleton_demo_release_catalog_index_matrix_smoke_wrapper_runs_without_editable_install(
    tmp_path,
):
    ...
    result = subprocess.run(
        [
            sys.executable,
            "scripts/run_singleton_demo_release_catalog_index_matrix_smoke.py",
            "--output-dir",
...
def test_module_entrypoint_singleton_demo_release_catalog_index_matrix_smoke_runs_with_repo_pythonpath(
    tmp_path,
):
    repo_root = Path(__file__).resolve().parents[1]
    result = subprocess.run(
