...
def test_load_machine_publication_registry_workspace_preset_example():
    ...
    assert preset["publication_registry_preset_path"] == str(
        (ROOT / "examples" / "registry_presets" / "machine_compute_publication_registry.json").resolve()
    )
...
def test_load_stable_publication_registry_workspace_preset_example():
    ...
    assert preset["publication_registry_preset_path"] == str(
        (ROOT / "examples" / "registry_presets" / "stable_reference_publication_registry.json").resolve()
    )
...
def test_load_publication_catalog_workspace_preset_example():
    ...
    assert preset["discover_under"] == [str((ROOT / "examples" / "generated_publication_network").resolve())]
    assert preset["recursive"] is True
    assert preset["publication_descriptor_index_preset_path"] == str(
        (ROOT / "examples" / "publication_descriptor_index_presets" / "ai_compute_publication_descriptor_index.json").resolve()
    )
    assert preset["publication_metadata_catalog_preset_path"] == str(
        (ROOT / "examples" / "publication_metadata_catalog_presets" / "ai_compute_publication_metadata_catalog.json").resolve()
    )
    assert preset["descriptor_index_metadata"]["label"] == "SATROOT AI Compute Workspace Descriptor Index"
    assert preset["publication_metadata_catalog_metadata"]["label"] == "SATROOT AI Compute Workspace Metadata Catalog"


def test_load_machine_publication_catalog_workspace_preset_example():
    preset = load_machine_publication_catalog_workspace_preset(
        ROOT / "examples" / "publication_catalog_workspace_presets" / "machine_compute_publication_catalog_workspace.json"
    )
    assert preset["artifact_paths"] == []
    assert preset["discover_under"] == [str((ROOT / "examples" / "generated_machine_publication_network").resolve())]
    assert preset["recursive"] is True
    assert preset["publication_descriptor_index_preset_path"] == str(
        (ROOT / "examples" / "publication_descriptor_index_presets" / "machine_compute_publication_descriptor_index.json").resolve()
    )
    assert preset["publication_metadata_catalog_preset_path"] == str(
        (ROOT / "examples" / "publication_metadata_catalog_presets" / "machine_compute_publication_metadata_catalog.json").resolve()
    )
    assert preset["descriptor_index_metadata"]["label"] == "SATROOT Machine Compute Workspace Descriptor Index"
    assert preset["publication_metadata_catalog_metadata"]["label"] == "SATROOT Machine Compute Workspace Metadata Catalog"


def test_load_stable_publication_catalog_workspace_preset_example():
    preset = load_stable_publication_catalog_workspace_preset(
        ROOT / "examples" / "publication_catalog_workspace_presets" / "stable_reference_publication_catalog_workspace.json"
    )
    assert preset["artifact_paths"] == []
    assert preset["discover_under"] == [str((ROOT / "examples" / "generated_stable_publication_network").resolve())]
    assert preset["recursive"] is True
    assert preset["publication_descriptor_index_preset_path"] == str(
        (ROOT / "examples" / "publication_descriptor_index_presets" / "stable_reference_publication_descriptor_index.json").resolve()
    )
    assert preset["publication_metadata_catalog_preset_path"] == str(
        (ROOT / "examples" / "publication_metadata_catalog_presets" / "stable_reference_publication_metadata_catalog.json").resolve()
    )
    assert preset["descriptor_index_metadata"]["label"] == "SATROOT Stable Reference Workspace Descriptor Index"
    assert preset["publication_metadata_catalog_metadata"]["label"] == "SATROOT Stable Reference Workspace Metadata Catalog"
...
def test_cli_bootstrap_machine_publication_stack_from_presets(tmp_path, capsys):
    ...
    write_json(
        release_catalog_preset,
        {
            ...
            "catalog": {
                "channel": "stable",
                "label": "Machine Publication Stack",
                "published_at": "2026-07-03T09:50:00Z",
            },
        },
    )
    output_dir = tmp_path / "machine_publication_stack"
...
def test_cli_bootstrap_machine_publication_stack_with_stack_preset_json(tmp_path, capsys):
    ...
    exit_code = main(
        [
            ...
            str(output_dir),
            "--label",
            "Embedded Machine Stack Override",
        ]
    )
    assert exit_code == 0
...
def test_cli_bootstrap_machine_publication_network_from_presets(tmp_path, capsys):
    ...
    write_json(
        stack_preset_b,
        {
            ...
            "release_catalog": {
                "channel": "beta",
                "label": "Machine Network Stack Beta",
                "published_at": "2026-07-03T10:50:00Z",
            },
        },
    )
    index_preset = tmp_path / "machine_network_index.json"
    write_json(
        index_preset,
        {
            "type": "SATROOT-RELEASE-CATALOG-INDEX-PRESET",
            ...
            "index": {
                "channel": "stable",
                "label": "Machine Publication Network",
                "published_at": "2026-07-03T11:00:00Z",
            },
        },
    )
    output_dir = tmp_path / "machine_publication_network"
    ...
    exit_code = main(
        [
            ...
            str(output_dir),
            "--label",
            "Machine Publication Network Override",
        ]
    )
    assert exit_code == 0
    ...
    assert summary["stack_count"] == 2
    assert summary["release_catalog_index"]["release_catalog_count"] == 2
    assert summary["release_catalog_index"]["index"]["label"] == "Machine Publication Network Override"
    assert {entry["workspace_name"] for entry in summary["workspaces"]} == {"machine_stack_a", "machine_stack_b"}

    alpha_summary = json.loads((output_dir / "stack_workspaces" / "machine_stack_a" / "summary.json").read_text(encoding="utf-8"))
    beta_summary = json.loads((output_dir / "stack_workspaces" / "machine_stack_b" / "summary.json").read_text(encoding="utf-8"))
...
def test_cli_bootstrap_machine_publication_network_with_network_preset_json(tmp_path, capsys):
    ...
    write_json(
        machine_catalog_preset,
        {
            ...
            "version": "0.1",
            "profiles": ["SATROOT-MACHINE-1"],
            "symbol_overrides": {"SATROOT-MACHINE-1": "MNETP1"},
            "name_overrides": {"SATROOT-MACHINE-1": "Machine Preset Network Catalog"},
            "release": {
                "channel": "stable",
                "label": "Machine Preset Network Release",
                "published_at": "2026-07-03T11:10:00Z",
            },
        },
    )
    stack_preset = tmp_path / "machine_stack_single.json"
    write_json(
        stack_preset,
        {
            "type": "SATROOT-PUBLICATION-STACK-PRESET",
    ...
    exit_code = main(
        [
            ...
            str(output_dir),
            "--label",
            "Embedded Machine Network Override",
        ]
    )
    assert exit_code == 0
...
def test_cli_publish_publication_registry_workspace_with_inventory_json(tmp_path, capsys):
    ...
    assert main(
        [
            ...
            str(output_dir),
            "--label",
            "Inventory Published Registry Workspace",
        ]
    ) == 0
...
def test_cli_publish_publication_registry_workspace_from_published_standalone_catalog_workspace(tmp_path, capsys):
    ...
    exit_code = main(
        [
            ...
            str(output_dir),
            "--label",
            "Published Registry Workspace From Published Standalone Catalog",
        ]
    )
    assert exit_code == 0
...
def test_cli_publish_publication_registry_workspace_with_preset(tmp_path, capsys):
    ...
    write_json(
        preset_path,
        {
            ...
            "publication_registry": {
                "channel": "network",
                "label": "Preset Published Registry Workspace",
    ...
    exit_code = main(
        [
            ...
            str(output_dir),
            "--label",
            "Override Published Registry Workspace",
        ]
    )
    assert exit_code == 0
    ...
    assert summary["source_publication_catalog_workspace_dir"] == str(catalog_workspace_dir.resolve())
    assert summary["source_publication_network_dir"] == str(network_dir.resolve())
    assert summary["publication_registry"]["index"]["label"] == "Override Published Registry Workspace"
    assert main(["publication-registry-workspace-lint", str(output_dir)]) == 0
    capsys.readouterr()
...
def test_cli_publish_machine_publication_registry_workspace_with_inventory_json(tmp_path, capsys):
    machine_catalog_preset = tmp_path / "machine_inventory_registry_catalog.json"
    write_json(
        machine_catalog_preset,
        {
            "type": "SATROOT-DEMO-CATALOG-PRESET",
            "version": "0.1",
            "profiles": ["SATROOT-MACHINE-1"],
            "symbol_overrides": {"SATROOT-MACHINE-1": "MIRNW1"},
            "name_overrides": {"SATROOT-MACHINE-1": "Machine Inventory Registry Network Catalog"},
            "release": {
                "channel": "machine",
                "label": "Machine Inventory Registry Network Release",
                "published_at": "2026-07-06T05:55:00Z",
            },
        },
    )
    stack_preset = tmp_path / "machine_inventory_registry_stack.json"
    write_json(
        stack_preset,
        {
            "type": "SATROOT-PUBLICATION-STACK-PRESET",
            ...
            "release_catalog": {
                "channel": "machine",
                "label": "Machine Inventory Registry Network Stack",
                "published_at": "2026-07-06T05:57:00Z",
            },
        },
    )
    network_dir = tmp_path / "machine_inventory_registry_network"
    assert main(
        [
            "bootstrap-machine-publication-network",
            "--stack-preset-json",
            ...
            str(network_dir),
            "--label",
            "Machine Inventory Registry Network",
        ]
    ) == 0
    capsys.readouterr()
    ...
    assert main(
        [
            ...
            str(output_dir),
            "--label",
            "Inventory Published Machine Registry Workspace",
        ]
    ) == 0
...
def test_cli_publish_machine_publication_registry_workspace_with_preset(tmp_path, capsys):
    ...
    write_json(
        preset_path,
        {
            ...
            "publication_registry": {
                "channel": "machine",
                "label": "Preset Machine Published Registry Workspace",
                "published_at": "2026-07-16T05:10:00Z",
            },
        },
    )
    ...
    exit_code = main(
        [
            ...
            str(output_dir),
            "--label",
            "Override Machine Published Registry Workspace",
        ]
    )
    assert exit_code == 0
...
def test_cli_publish_machine_publication_registry_workspace_from_published_standalone_catalog_workspace(tmp_path, capsys):
    ...
    exit_code = main(
        [
            ...
            str(output_dir),
            "--label",
            "Published Machine Registry Workspace From Published Standalone Catalog",
        ]
    )
    assert exit_code == 0
...
def test_cli_bootstrap_machine_publication_registry_workspace_hmac(tmp_path, capsys):
    ...
    exit_code = main(
        [
            ...
            "APIPUBREG1",
            "--name",
            "Machine Publication Registry CLI",
            "--scheme",
            "hmac-sha256",
            "--release-key-id",
            "release-key",
...
def test_cli_bootstrap_publication_registry_workspace_with_preset_json_and_cli_overrides(tmp_path, capsys):
    ...
    write_json(
        preset_path,
        {
            ...
            "publication_registry": {
                "channel": "network",
                "label": "Preset Workspace Publication Registry",
                "published_at": "2026-07-08T07:10:00Z",
            },
        },
    )
    output_dir = tmp_path / "publication_registry_workspace_preset"
    ...
    exit_code = main(
        [
            ...
            "Override Workspace Descriptor Index",
            "--publication-registry-label",
            "Override Workspace Publication Registry",
        ]
    )
    assert exit_code == 0
...
def test_cli_bootstrap_publication_registry_workspace_from_publication_catalog_workspace(tmp_path, capsys):
    network_dir = make_demo_publication_network_dir(tmp_path)
    catalog_workspace_dir = make_publication_catalog_workspace_dir(tmp_path)
    output_dir = tmp_path / "publication_registry_workspace_from_catalog"
    ...
    exit_code = main(
        [
            ...
            "--publication-network-dir",
            str(network_dir),
            "--publication-catalog-workspace-dir",
            str(catalog_workspace_dir),
            "--scheme",
            "hmac-sha256",
            "--publication-descriptor-index-key-id",
...
def test_cli_bootstrap_publication_registry_workspace_with_preset_catalog_workspace_reference(tmp_path, capsys):
    ...
    output_dir = tmp_path / "publication_registry_workspace_catalog_preset"
...
def test_cli_export_publication_registry_workspace_preset_preserves_catalog_workspace_reference(tmp_path):
    ...
    preset_path = tmp_path / "exported_registry_workspace_from_catalog.json"
    exit_code = main(["export-publication-registry-workspace-preset", str(workspace_dir), "--output", str(preset_path)])
    assert exit_code == 0
    ...
    loaded = load_publication_registry_workspace_preset(preset_path)
    assert preset["type"] == "SATROOT-PUBLICATION-REGISTRY-WORKSPACE-PRESET"
    assert Path(loaded["publication_catalog_workspace_dir"]).name == "publication_catalog_workspace"
    assert Path(loaded["publication_network_dir"]).name == "publication_network"
    assert preset["publication_registry"]["label"] == "Workspace Publication Registry"
...
def test_cli_publish_publication_descriptor_index_with_inventory_json(tmp_path, capsys):
    network_dir = make_demo_publication_network_dir(tmp_path)
    inventory_path = tmp_path / "publication_descriptor_index_inventory.json"
    ...
    assert (
        main(
            [
                ...
                "network",
                "--label",
                "Inventory Published Descriptor Index",
                "--published-at",
                "2026-07-09T02:15:00Z",
                "--scheme",
                "hmac-sha256",
                "--key-id",
                "descriptor-key",
    ...
    assert index["artifact_count"] == 12
    assert index["artifact_kind_counts"]["publication-network"] == 1
    assert index["index"]["label"] == "Inventory Published Descriptor Index"
    assert main(["publication-descriptor-index-lint", str(output_dir)]) == 0
    capsys.readouterr()
...
def test_cli_publish_machine_publication_descriptor_index_with_example_preset(tmp_path, capsys):
    ...
    assert main(
        [
            ...
            str(generated_network_dir),
            "--label",
            "Generated Machine Network For Descriptor Preset",
        ]
    ) == 0
    capsys.readouterr()
    ...
    assert (
        main(
            [
                ...
                "descriptor-secret",
                "--label",
                "Preset Published Machine Descriptor Index Override",
            ]
        )
        == 0
    )
...
def test_cli_publish_stable_publication_descriptor_index_with_example_preset(tmp_path, capsys):
    ...
    output_dir = tmp_path / "stable_publication_descriptor_index_publication_preset"
    ...
    assert main(
        [
            ...
            str(generated_network_dir),
            "--label",
            "Generated Stable Network For Descriptor Preset",
        ]
    ) == 0
    capsys.readouterr()
    ...
    assert (
        main(
            [
                ...
                "descriptor-secret",
                "--label",
                "Preset Published Stable Descriptor Index Override",
            ]
        )
        == 0
    )
...
def test_cli_bootstrap_publication_descriptor_index_publication_with_example_preset(tmp_path, capsys):
    ...
    generated_network_dir = tmp_path / "examples" / "generated_publication_network"
    output_dir = tmp_path / "publication_descriptor_index_example_preset"
    ...
    assert main(
        [
            ...
            "descriptor-key",
            "--label",
            "Example Descriptor Index Override",
        ]
    ) == 0
    ...
    assert index["artifact_kind_counts"]["publication-network"] == 1
    assert index["index"]["channel"] == "network"
    assert index["index"]["label"] == "Example Descriptor Index Override"
    assert index["index"]["published_at"] == "2026-07-08T02:00:00Z"
    assert verified["artifact_count"] == index["artifact_count"]
    assert main(["publication-descriptor-index-lint", str(output_dir)]) == 0
    capsys.readouterr()
...
def test_cli_bootstrap_machine_publication_descriptor_index_publication_with_example_preset(tmp_path, capsys):
    ...
    generated_network_dir = tmp_path / "examples" / "generated_machine_publication_network"
    output_dir = tmp_path / "machine_publication_descriptor_index_example_preset"
    ...
    assert main(
        [
            "bootstrap-machine-publication-descriptor-index-publication",
            "--preset-json",
            str(staged["publication_descriptor_index_presets/machine_compute_publication_descriptor_index.json"]),
            "--output-dir",
            str(output_dir),
            "--scheme",
            "hmac-sha256",
            ...
            "descriptor-key",
            "--label",
            "Machine Example Descriptor Index Override",
        ]
    ) == 0
    ...
    assert index["artifact_kind_counts"]["publication-network"] == 1
    assert index["index"]["channel"] == "machine"
    assert index["index"]["label"] == "Machine Example Descriptor Index Override"
    assert index["index"]["published_at"] == "2026-07-14T04:30:00Z"
    assert verified["artifact_count"] == index["artifact_count"]
    assert main(["publication-descriptor-index-lint", str(output_dir)]) == 0
    capsys.readouterr()
...
def test_cli_bootstrap_stable_publication_descriptor_index_publication_with_example_preset(tmp_path, capsys):
    ...
    generated_network_dir = tmp_path / "examples" / "generated_stable_publication_network"
    output_dir = tmp_path / "stable_publication_descriptor_index_example_preset"
    ...
    assert main(
        [
            "bootstrap-stable-publication-descriptor-index-publication",
            "--preset-json",
            str(staged["publication_descriptor_index_presets/stable_reference_publication_descriptor_index.json"]),
            "--output-dir",
            str(output_dir),
            "--scheme",
            "hmac-sha256",
            ...
            "descriptor-key",
            "--label",
            "Stable Example Descriptor Index Override",
        ]
    ) == 0
    ...
    assert index["artifact_kind_counts"]["publication-network"] == 1
    assert index["index"]["channel"] == "stable"
    assert index["index"]["label"] == "Stable Example Descriptor Index Override"
    assert index["index"]["published_at"] == "2026-07-15T04:30:00Z"
    assert verified["artifact_count"] == index["artifact_count"]
    assert main(["publication-descriptor-index-lint", str(output_dir)]) == 0
    capsys.readouterr()
...
def test_cli_bootstrap_publication_metadata_bundle_for_catalog_workspace(tmp_path, capsys):
    workspace_dir = make_publication_catalog_workspace_dir(tmp_path)
    output_dir = tmp_path / "publication_metadata_catalog_workspace_bundle"
...
def test_cli_publish_publication_metadata_catalog_with_inventory_json(tmp_path, capsys):
    _release_bundle_dir, _network_bundle_dir = make_publication_metadata_bundle_dirs(tmp_path)
    inventory_path = tmp_path / "publication_metadata_catalog_inventory.json"
    ...
    assert (
        main(
            [
                ...
                "network",
                "--label",
                "Inventory Published Metadata Catalog",
                "--published-at",
                "2026-07-09T04:15:00Z",
                "--scheme",
                "hmac-sha256",
                "--key-id",
                "catalog-key",
    ...
    assert catalog["artifact_kind_counts"]["release"] == 1
    assert catalog["artifact_kind_counts"]["publication-network"] == 1
    assert catalog["index"]["label"] == "Inventory Published Metadata Catalog"
    assert main(["publication-metadata-catalog-lint", str(output_dir)]) == 0
    capsys.readouterr()
...
def test_cli_publish_machine_publication_metadata_catalog_with_example_preset(tmp_path, capsys):
    ...
    assert (
        main(
            [
                ...
                "catalog-secret",
                "--label",
                "Preset Published Machine Metadata Catalog Override",
            ]
        )
        == 0
    )
    ...
    assert catalog["bundle_count"] == 5
    assert catalog["artifact_kind_counts"]["bundle"] == 1
    assert catalog["artifact_kind_counts"]["release"] == 1
    assert catalog["artifact_kind_counts"]["release-catalog"] == 1
    assert catalog["artifact_kind_counts"]["demo-catalog"] == 1
    assert catalog["artifact_kind_counts"]["publication-catalog-workspace"] == 1
    assert catalog["index"]["channel"] == "machine"
    assert catalog["index"]["label"] == "Preset Published Machine Metadata Catalog Override"
    assert catalog["index"]["published_at"] == "2026-07-14T05:00:00Z"
...
def test_cli_publish_stable_publication_metadata_catalog_with_example_preset(tmp_path, capsys):
    ...
    assert (
        main(
            [
                ...
                "catalog-secret",
                "--label",
                "Preset Published Stable Metadata Catalog Override",
            ]
        )
        == 0
    )
    ...
    assert catalog["artifact_kind_counts"]["bundle"] == 1
    assert catalog["artifact_kind_counts"]["release"] == 1
    assert catalog["artifact_kind_counts"]["demo-catalog"] == 2
    assert catalog["artifact_kind_counts"]["publication-catalog-workspace"] == 1
    assert catalog["index"]["channel"] == "stable"
    assert catalog["index"]["label"] == "Preset Published Stable Metadata Catalog Override"
    assert catalog["index"]["published_at"] == "2026-07-15T05:00:00Z"
    assert verified["bundle_count"] == 5
    assert main(["publication-metadata-catalog-lint", str(output_dir)]) == 0
    capsys.readouterr()
...
def test_cli_bootstrap_publication_metadata_catalog_publication_with_example_preset(tmp_path, capsys):
    ...
    assert main(
        [
            ...
            "catalog-key",
            "--label",
            "Example Metadata Catalog Override",
        ]
    ) == 0
    ...
    assert catalog["artifact_kind_counts"]["publication-network"] == 1
    assert catalog["index"]["channel"] == "network"
    assert catalog["index"]["label"] == "Example Metadata Catalog Override"
    assert catalog["index"]["published_at"] == "2026-07-08T04:30:00Z"
    assert verified["bundle_count"] == 2
    assert main(["publication-metadata-catalog-lint", str(output_dir)]) == 0
    capsys.readouterr()
...
def test_cli_bootstrap_machine_publication_metadata_catalog_publication_with_example_preset(tmp_path, capsys):
    ...
    assert main(
        [
            ...
            "catalog-key",
            "--label",
            "Machine Example Metadata Catalog Override",
        ]
    ) == 0
    ...
    assert catalog["artifact_kind_counts"]["publication-catalog-workspace"] == 1
    assert catalog["index"]["channel"] == "machine"
    assert catalog["index"]["label"] == "Machine Example Metadata Catalog Override"
    assert catalog["index"]["published_at"] == "2026-07-14T05:00:00Z"
    assert verified["bundle_count"] == 5
    assert main(["publication-metadata-catalog-lint", str(output_dir)]) == 0
    capsys.readouterr()
...
def test_cli_bootstrap_stable_publication_metadata_catalog_publication_with_preset_json_and_cli_overrides(tmp_path, capsys):
    ...
    output_dir = tmp_path / "stable_publication_metadata_catalog_preset_publication"
    ...
    assert main(
        [
            ...
            "catalog-key",
            "--label",
            "CLI Stable Metadata Catalog Override",
        ]
    ) == 0
    ...
    assert catalog["bundle_count"] == 2
    assert catalog["index"]["channel"] == "stable"
    assert catalog["index"]["label"] == "CLI Stable Metadata Catalog Override"
    assert catalog["index"]["published_at"] == "2026-07-15T05:45:00Z"
...
def test_cli_bootstrap_stable_publication_metadata_catalog_publication_with_example_preset(tmp_path, capsys):
    ...
    assert main(
        [
            ...
            "catalog-key",
            "--label",
            "Stable Example Metadata Catalog Override",
        ]
    ) == 0
    ...
    assert catalog["artifact_kind_counts"]["publication-catalog-workspace"] == 1
    assert catalog["index"]["channel"] == "stable"
    assert catalog["index"]["label"] == "Stable Example Metadata Catalog Override"
    assert catalog["index"]["published_at"] == "2026-07-15T05:00:00Z"
    assert verified["bundle_count"] == 5
    assert main(["publication-metadata-catalog-lint", str(output_dir)]) == 0
    capsys.readouterr()
    ...
    assert (
        main(
            [
                ...
                "network",
                "--label",
                "Inventory Published Publication Registry",
                "--published-at",
                "2026-07-09T05:15:00Z",
                "--scheme",
                "hmac-sha256",
                "--key-id",
                "registry-key",
...
def test_cli_publish_machine_publication_registry_with_example_preset(tmp_path, capsys):
    ...
    output_dir = tmp_path / "machine_publication_registry_publication_preset"
    ...
    assert main(
        [
            ...
            str(generated_network_dir),
            "--label",
            "Generated Machine Network For Registry Preset",
        ]
    ) == 0
    assert main(
        [
            ...
            "machine",
            "--label",
            "Generated Machine Descriptor Index For Registry Preset",
            "--scheme",
            "hmac-sha256",
            "--key-id",
            "descriptor-key",
    ...
    assert main(
        [
            ...
            "machine",
            "--label",
            "Generated Machine Metadata Catalog For Registry Preset",
            "--scheme",
            "hmac-sha256",
            "--key-id",
            "catalog-key",
    ...
    assert (
        main(
            [
                ...
                "registry-secret",
                "--label",
                "Preset Published Machine Publication Registry Override",
            ]
        )
        == 0
    )
...
def test_cli_publish_stable_publication_registry_with_example_preset(tmp_path, capsys):
    ...
    output_dir = tmp_path / "stable_publication_registry_publication_preset"
    ...
    assert main(
        [
            ...
            str(generated_network_dir),
            "--label",
            "Generated Stable Network For Registry Preset",
        ]
    ) == 0
    assert main(
        [
            ...
            "stable",
            "--label",
            "Generated Stable Descriptor Index For Registry Preset",
            "--scheme",
            "hmac-sha256",
            "--key-id",
            "descriptor-key",
    ...
    assert main(
        [
            ...
            "stable",
            "--label",
            "Generated Stable Metadata Catalog For Registry Preset",
            "--scheme",
            "hmac-sha256",
            "--key-id",
            "catalog-key",
    ...
    assert (
        main(
            [
                ...
                "registry-secret",
                "--label",
                "Preset Published Stable Publication Registry Override",
            ]
        )
        == 0
    )
...
def test_cli_bootstrap_publication_registry_publication_with_example_preset(tmp_path, capsys):
    ...
    generated_metadata_root = tmp_path / "examples" / "generated_publication_metadata_root"
    generated_metadata_dir = tmp_path / "examples" / "generated_publication_metadata_catalog_publication"
    output_dir = tmp_path / "publication_registry_example_preset"
    ...
    assert main(
        [
            ...
            "registry-key",
            "--label",
            "Example Publication Registry Override",
        ]
    ) == 0
...
def test_cli_bootstrap_machine_publication_registry_publication_with_example_preset(tmp_path, capsys):
    ...
    generated_metadata_root = tmp_path / "examples" / "generated_machine_publication_metadata_root"
    generated_metadata_dir = tmp_path / "examples" / "generated_machine_publication_metadata_catalog_publication"
    output_dir = tmp_path / "machine_publication_registry_example_preset"
    ...
    assert main(
        [
            "bootstrap-machine-publication-metadata-catalog-publication",
            "--preset-json",
            str(staged["publication_metadata_catalog_presets/machine_compute_publication_metadata_catalog.json"]),
            "--output-dir",
            str(generated_metadata_dir),
            "--scheme",
            "hmac-sha256",
    ...
    assert main(
        [
            "bootstrap-machine-publication-registry-publication",
            "--preset-json",
            str(staged["registry_presets/machine_compute_publication_registry.json"]),
            "--output-dir",
            str(output_dir),
            "--scheme",
            "hmac-sha256",
            ...
            "registry-key",
            "--label",
            "Machine Example Publication Registry Override",
        ]
    ) == 0
...
def test_cli_bootstrap_stable_publication_registry_publication_with_preset_json_and_cli_overrides(tmp_path, capsys):
    ...
    write_json(
        preset_path,
        {
            ...
            "registry": {
                "channel": "stable",
                "label": "Preset Stable Publication Registry",
                "published_at": "2026-07-15T06:45:00Z",
            },
        },
    )
    output_dir = tmp_path / "stable_publication_registry_preset_publication"
    ...
    assert main(
        [
            ...
            "registry-key",
            "--label",
            "CLI Stable Publication Registry Override",
        ]
    ) == 0
    ...
    assert registry["component_count"] == 3
    assert registry["index"]["channel"] == "stable"
    assert registry["index"]["label"] == "CLI Stable Publication Registry Override"
    assert registry["index"]["published_at"] == "2026-07-15T06:45:00Z"
...
def test_cli_bootstrap_stable_publication_registry_publication_with_example_preset(tmp_path, capsys):
    ...
    generated_metadata_root = tmp_path / "examples" / "generated_stable_publication_metadata_root"
    generated_metadata_dir = tmp_path / "examples" / "generated_stable_publication_metadata_catalog_publication"
    output_dir = tmp_path / "stable_publication_registry_example_preset"
    ...
    assert main(
        [
            "bootstrap-stable-publication-metadata-catalog-publication",
            "--preset-json",
            str(staged["publication_metadata_catalog_presets/stable_reference_publication_metadata_catalog.json"]),
            "--output-dir",
            str(generated_metadata_dir),
            "--scheme",
            "hmac-sha256",
    ...
    assert main(
        [
            "bootstrap-stable-publication-registry-publication",
            "--preset-json",
            str(staged["registry_presets/stable_reference_publication_registry.json"]),
            "--output-dir",
            str(output_dir),
            "--scheme",
            "hmac-sha256",
            ...
            "registry-key",
            "--label",
            "Stable Example Publication Registry Override",
        ]
    ) == 0
    ...
    assert registry["component_count"] == 3
    assert registry["index"]["channel"] == "stable"
    assert registry["index"]["label"] == "Stable Example Publication Registry Override"
    assert registry["index"]["published_at"] == "2026-07-15T06:00:00Z"
    assert verified["component_count"] == 3
    assert main(["publication-registry-lint", str(output_dir)]) == 0
    capsys.readouterr()
