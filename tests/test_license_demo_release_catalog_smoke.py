...
def test_run_license_demo_release_catalog_smoke_builds_collection_and_catalog(tmp_path):
    report = run_license_demo_release_catalog_smoke(tmp_path / "license_demo_release_catalog_smoke")
    ...
    assert report["release_collection"]["bundle_symbols"] == ["LICCAT01", "LICCAT02"]
...
def test_repo_local_license_demo_release_catalog_smoke_wrapper_runs_without_editable_install(tmp_path):
    repo_root = Path(__file__).resolve().parents[1]
    result = subprocess.run(
        [
            sys.executable,
            "scripts/run_license_demo_release_catalog_smoke.py",
            "--output-dir",
...
def test_module_entrypoint_license_demo_release_catalog_smoke_runs_with_repo_pythonpath(tmp_path):
    repo_root = Path(__file__).resolve().parents[1]
    result = subprocess.run(
        [
            sys.executable,
