...
- [ ] Run `python scripts/run_operator_proof_smoke.py` from the repo root, or use `python -m satroot_operator_proof_smoke` / `satroot-operator-proof-smoke` after `pip install -e .`, as the top-level released operator proof across the stable/machine publication ladder, singleton publication ladder, and mixed-profile federation surface.
