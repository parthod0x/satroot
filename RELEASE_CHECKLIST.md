...
- [ ] After `pip install -e .`, run `python -c "import satroot1, satroot_collection_lint, satroot_test, satroot_stable_profile_smoke, satroot_machine_profile_smoke, satroot_receipt_profile_smoke"` to confirm the installed modules resolve outside the repo-local pytest `pythonpath` shortcut.
