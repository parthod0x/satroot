...
- [ ] Optionally run `python scripts/run_profile_federation_smoke.py` from the repo root, or use `python -m satroot_profile_federation_smoke` / `satroot-profile-federation-smoke` after `pip install -e .`, to verify the first mixed-profile publication-federation wrapper above the released per-profile lanes.
