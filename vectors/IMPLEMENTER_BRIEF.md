...
ledger, from the specification alone, and run it against the 63 conformance
...
63-vector conformance corpus. All of it was written by one person. So the
...
grew from 33 vectors to 63 across three rounds, closing the gaps that run exposed. **Assume more
...
cd satroot/vectors
python3 run.py            # expect: 63 vectors, 0 failures
...
Run it. You should see **48 ok, 15 FAIL** — the 48 rejection vectors pass,
...
|---|---|---|
| stub above | — | **48** |
| canonical JSON, `event_id`, genesis, `demo` verifier, state hash | §2.6, §2.7, §5, §5.1, §5.2, §6.6, §7 | 51 |
| transfer | §6.2, §6.1a, §6.7 | 54 |
| mint | §6.1 | 56 |
| burn | §6.3, §6.7 | 57 |
| freeze / unfreeze | §6.5 | 59 |
| rotate-authority | §6.4 | 61 |
| `hmac-sha256` verifier | §6.6.1 | 62 |
| `ed25519` verifier | §6.6.1 | **63** |

Cross-check the rejection conditions in §8 throughout.

**48 → 51 is by far the hardest step.** It means your canonical JSON, event
...
57 of the 63 vectors use the `demo` scheme, so real cryptography can wait
until the very end.
